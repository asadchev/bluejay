...
namespace eigen {
  ...
  template<typename T>
  std::unique_ptr< Matrix<T> > make_matrix(py::buffer b) {

    typedef typename Matrix<T>::Scalar Scalar;
    typedef Eigen::Stride<Eigen::Dynamic, Eigen::Dynamic> Strides;
    constexpr bool rowMajor = (Matrix<T>::Flags & Eigen::RowMajorBit);
    ...
    if (info.format != py::format_descriptor<Scalar>::value) {
      throw Exception(
        "Matrix: incompatible data type, expected %s",
        boost::typeindex::type_id<T>().pretty_name()
      );
    }

    if (info.ndim != 2) {
      throw Exception("Matrix: incompatible buffer dimension");
    }
    ...
    auto map = Eigen::Map<Matrix<T>, 0, Strides>(
    ...
    return std::make_unique< Matrix<T> >(map);
  ...
  py::object __repr__(const Matrix<T> &m) {
    std::string dtype = std::type_index(typeid(T)).name();
    std::string repr = "Matrix("
      "dtype=" + dtype +
      ", rows=" + str(m.rows()) +
      ", cols=" + str(m.cols()) +
      ")";
  ...
  void init(py::module m) {
    ...
    //   .def(py::init(&make_matrix<double>))
    ...
    //py::implicitly_convertible<Tensor4d, py::buffer>();
